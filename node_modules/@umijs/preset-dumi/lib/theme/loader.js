...
function getThemeResolvePath(sourcePath) {
  return (0, _moduleResolver.getModuleResolvePath)({
    // start search theme from @umijs/preset-dumi package, but use cwd if use relative theme folder
    basePath: sourcePath.startsWith('.') ? _context.default.umi.cwd : __dirname,
    sourcePath,
    silent: true
  });
}
...
var _default = /*#__PURE__*/_asyncToGenerator(function* () {
  if (!cache || process.env.NODE_ENV === 'test') {
    ...
    const fallback = fb.startsWith('.') ? (0, _utils().winPath)(_path().default.dirname(getThemeResolvePath(fb))) : fb;
    const modulePath = _path().default.isAbsolute(theme) ? theme : // resolve real absolute path for theme package
    (0, _utils().winPath)(_path().default.dirname(getThemeResolvePath(theme))); // local theme has no src directory but theme package has

    const srcPath = _path().default.isAbsolute(theme) ? theme : `${modulePath}/src`;
    ...
    const components = _fs().default.existsSync(builtinPath) ? _fs().default.readdirSync(builtinPath).filter(file => /\.(j|t)sx?$/.test(file)).map(file => ({
      ...
      source: theme.startsWith('.') ? // use abs path for relative theme folder
      pathJoin(builtinPath, file) : // still use module identifier rather than abs path for theme package and absolute theme folder
    ...
    const fallbacks = REQUIRED_THEME_BUILTINS.reduce((result, bName) => {
      if (components.every(({
        ...
        try {
          ...
          debug('fallback to default theme for:', cSource); // fallback to default theme if detected fallback theme missed some components
    ...
    [pathJoin(srcPath, 'layout'), pathJoin(srcPath, 'layouts'), pathJoin(fallback, 'src', 'layout'), pathJoin(fallback, 'src', 'layouts')].some((layoutPath, i, outerLayoutPaths) => {
      try {
    ...
    [pathJoin(srcPath, 'layouts', 'demo'), pathJoin(fallback, 'src', 'layouts', 'demo')].some(layoutPath => {
      try {
