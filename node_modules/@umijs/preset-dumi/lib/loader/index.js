...
function _loader() {
  _loader = _asyncToGenerator(function* (raw) {
    ...
    const theme = yield (0, _loader2.default)(); // mark current file if it contains Katex and there has not another file used Katex
    ...
    useKatexFilePath === this.resource || process.env.NODE_ENV !== 'production' ? "import 'katex/dist/katex.min.css';" : ''}
    ...
    export default function () {
      return (
        <>
          ${result.meta.translateHelp ? result.meta.translateHelp === true ? `<Alert>This article has not been translated yet. Want to help us out? Click the Edit this doc on GitHub at the end of the page.</Alert>` : `<Alert>${result.meta.translateHelp}</Alert>` : ''}
          ${result.content}
        </>
      );
  }`;
