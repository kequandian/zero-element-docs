...
function serializeAPINodes(node, identifier, definitions) {
  ...
  return expts.reduce((list, expt, i) => {
    ...
    const isInsertAPITitle = expt === 'default' || !i && !parsedAttrs.exports; // render sub title for non-default export

    const isInsertSubTitle = expt !== 'default';
    const apiNode = (0, _deepmerge().default)({}, node); // insert API title
...
function guessComponentName(fileAbsPath) {
  ...
  if (parsed.name === 'index') {
    ...
    return _path().default.basename(parsed.dir.replace(/\/src$/, ''));
...
function watchComponentUpdate(absPath, componentName, identifier) {
  (0, _watcher.listenFileOnceChange)(absPath, () => {
    let definitions;

    try {
      definitions = (0, _apiParser.default)(absPath, componentName);
    } catch (err) {
      /* noting */
    } // update api data


    applyApiData(identifier, definitions); // watch next turn
    ...
    (0, _unistUtilVisit().default)(ast, 'element', (node, i, parent) => {
      if ((0, _hastUtilIsElement().default)(node, 'API') && !node._dumi_parsed) {
        ...
        if ((0, _hastUtilHasProperty().default)(node, 'src')) {
          const src = node.properties.src || ''; // guess component name if there has no identifier property

          const componentName = node.properties.identifier || guessComponentName(src);

          const absPath = _path().default.join(_path().default.dirname(this.data('fileAbsPath')), src);

          definitions = (0, _apiParser.default)(absPath, componentName);
          identifier = componentName || src; // trigger listener to update previewer props after this file changed
          ...
          try {
            ...
            definitions = (0, _apiParser.default)(sourcePath, vFile.data.componentName);
            identifier = vFile.data.componentName; // trigger listener to update previewer props after this file changed
        ...
        if (identifier && definitions) {
          ...
          parent.children.splice(i, 1, ...serializeAPINodes(node, identifier, definitions)); // apply api data
