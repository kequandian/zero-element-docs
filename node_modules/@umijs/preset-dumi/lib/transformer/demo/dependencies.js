...
function analyzeDeps(raw, {
      ...
      CallExpression(callPath) {
        ...
        if (types().isIdentifier(callPathNode.callee) && callPathNode.callee.name === 'require' && types().isStringLiteral(callPathNode.arguments[0])) {
          ...
          requireStr.startsWith('.')) {
            // save local deps
            const filename = (0, _slash().default)(_path().default.relative(fileAbsPath, resolvePath)).replace(/(\.\/|\..\/)/g, '');
            cache.files.push({
              resolvePath,
              requireStr,
              filename
  ...
  cache.files.filter(item => {
    ...
    if (depChangeListener) {
      (0, _watcher.listenFileOnceChange)(item.resolvePath, depChangeListener);
    }
  ...
  if (depChangeListener) {
    (0, _watcher.listenFileOnceChange)(fileAbsPath, depChangeListener);
  }
