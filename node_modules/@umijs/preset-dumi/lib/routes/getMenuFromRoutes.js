    ...
    if (menuItem.children) {
      menuItem.children = menu.children.map(child => {
        ...
        if (typeof child === 'string') {
          ...
          route.meta = route.meta || {}; // update original route group
...
function menuSorter(prev, next) {
  ...
  const nextOrder = typeof ((_next$meta = next.meta) === null || _next$meta === void 0 ? void 0 : _next$meta.order) === 'number' ? next.meta.order : Infinity; // compare order meta config first

  const metaOrder = prevOrder === nextOrder ? 0 : prevOrder - nextOrder; // last compare path ASCII
  ...
  routes.forEach(route => {
    if (isValidMenuRoutes(route)) {
      ...
      if (group === null || group === void 0 ? void 0 : group.path) {
        var _localeMenusMapping$l, _localeMenusMapping$l2, _localeMenusMapping$l3, _localeMenusMapping$l4, _localeMenusMapping$l5, _localeMenusMapping$l6, _localeMenusMapping$l7;

        const title = group.title,
              groupPath = group.path,
              meta = _objectWithoutProperties(group, ["title", "path"]);

        const groupKey = addHtmlSuffix(groupPath) || title; // group route items by group path & locale
  ...
  Object.keys(userMenus).forEach(navPath => {
    [...opts.locales].reverse().some(locale => {
      ...
      if (navPath.startsWith(localePrefix)) {
        ...
        localeMenus[locale[0]][navPath] = convertedMenus; // update original nav index page
