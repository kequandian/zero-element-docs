...
var _default = (routes, opts, userCustomNavs) => {
  ...
  Object.keys(localeNavs).forEach(key => {
    localeNavs[key] = Object.values(localeNavs[key]).sort((prev, next) => {
      ...
      const nextOrder = typeof next.order === 'number' ? next.order : Infinity; // compare order meta config first

      const metaOrder = prevOrder === nextOrder ? 0 : prevOrder - nextOrder; // last compare path length

      const pathOrder = prev.path.length - next.path.length; // then compare title ASCII
  ...
  Object.keys(localeNavs).forEach(locale => {
    if (customNavs[locale]) {
      localeNavs[locale] = customNavs[locale].reduce( // concat original navs if navs has empty item from user
