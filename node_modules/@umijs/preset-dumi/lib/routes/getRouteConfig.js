...
var _default = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (api, opts) {
    ...
    const userRoutes = opts.isIntegrate ? (_yield$api$applyPlugi = yield api.applyPlugins({
    ...
    if (userRoutes) {
      ...
      childRoutes.push(...userRoutes.map((_ref2) => {
        let component = _ref2.component,
            routeOpts = _objectWithoutProperties(_ref2, ["component"]);
    ...
    config.push({
      ...
      wrappers: [// builtin outer layout
      (0, _slash().default)(_path().default.relative(api.paths.absPagesPath, _path().default.join(__dirname, '../theme/layout'))), // theme layout
      (0, _slash().default)(_path().default.relative(api.paths.absPagesPath, theme.layoutPaths._))],
      // decorate standard umi routes
      routes: (0, _decorator.default)(childRoutes, opts, api),
      title: opts.title
    });
    debug('decorateRoutes'); // process example routes

    config[0].routes.forEach(route => {
      var _route$meta;

      if ((_route$meta = route.meta) === null || _route$meta === void 0 ? void 0 : _route$meta.example) {
        const examplePath = route.path.replace('/', exampleRoutePrefix); // add example into top-level routes for external openning
